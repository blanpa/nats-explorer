...
A chart is fetched from the server already reduced -- up to 10 000 messages per subject, from memory and, where memory falls short, from the database -- and it keeps moving between two answers: min/max appends every message as it arrives, and the other reductions continue in the buckets the server used, a point per bucketful, so an average never stands next to a raw value. Only the minute rollup waits for the server, whose buckets are the clock's.

- **Delay from the producer's clock**: where a payload carries the moment it was made, the difference to its arrival is charted like any other field -- the one figure that says whether a hold-up is upstream of the explorer. Only a value near the message's own time counts as its clock, and a negative delay is shown as it is: a producer running ahead is a clock problem, not a healthy zero
...
- Drag across a chart to zoom into that stretch of time. The drag sets the time range, so the history, the search, the schema and an export follow it, and the server reduces the field over the shorter window -- zooming in really does show more, it does not stretch what was already drawn. The window is named in the range row as soon as it is chosen, and "Custom" always opens on what is on screen -- the dragged window to the second, the selected preset, or the last hour while live -- so a rough drag can be corrected rather than typed out again
- A silence is drawn as a silence: where nothing was sent for more than about four times the usual distance, the line stops and starts again on the other side. Joining the two ends would draw a steady ramp across the outage, which is exactly what a healthy sensor looks like
...
Whether a number is whole is read from how it was written, not from what it happens to be: `21` is an integer, `21.0` and `1e3` are not, and a field that has carried both is a number. JSON has one number type, and a field taken for an integer because the sensor sat on 21.0 all morning is a schema that rejects the first 21.5. A string field is named `date-time` when every sample of it was an RFC 3339 timestamp. Payloads with more than 200 paths are reported as far as they go and say so, rather than letting a cut list pass for the whole shape.
