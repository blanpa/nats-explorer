...
| `HISTORY_RETENTION` | `72h` | Rows older than this are deleted once a minute (Go duration, e.g. `24h`, `168h`); `0` keeps every message and deletes nothing. Without `HISTORY_DB` it is the starting value of the setting, which then wins once changed |
