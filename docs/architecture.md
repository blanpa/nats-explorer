...
- Answers two queries: the messages of one subject (oldest first, pageable backwards by sequence) and the newest messages below a branch (a k-way merge over the subjects' FIFOs).
- `GET /api/history/series` extracts a numeric JSON field over a subject's recorded history and downsamples it to min/max buckets, so a chart over 10 000 messages costs a few hundred points on the wire.
- With `HISTORY_DB` a `Tee` keeps the memory store as the live source and copies every record to SQLite (`internal/history/sqlite.go`): asynchronous, batched inserts of up to 2 000 rows per transaction, a bounded queue that drops rather than blocks when the writer falls behind (counted in the stats), retention by age once a minute. Time ranges, ranged search and ranged series read from it (`from`/`to` on `/api/history/range`, `/search`, `/series`).
...
| `subject-tree`      | Server    | Nodes of this tab's view (`{s,n,r,t,tr,c,p,pt,ts,sz}` = subject, own count and rate, subtree total and rate, child count, payload preview, type, timestamp, size) that appeared or changed, plus `removed` subjects that left the view. `full: true` replaces the connection's tree (first update of a tab, and after a subscription change). The browser merges nodes across connections. Interval grows with the tree (0.5 s → 1 s above 2k subjects → 2 s above 10k); view changes are answered at once |
...
| `view`              | Client    | What part of the tree to send: `all` (every branch, `paths` = collapsed exceptions) or the expanded `paths`, or a `filter` whose matches' paths are shown regardless of expansion. Nothing is sent before the first view |
...
| Method | Endpoint                   | Description |
|:------ |:-------------------------- |:----------- |
| GET    | `/api/history`             | Messages of a subject and, with `branchLimit`, of everything below it. `expr` filters with a CEL expression |
| GET    | `/api/history/range`       | Persisted messages in a time range (needs `HISTORY_DB`) |
| GET    | `/api/history/search`      | Subject or payload text, over one subject or, without `subject`, over all of them |
...
| DELETE | `/api/history`             | Forget the recorded messages; with `subject` only that one, with `branch=1` everything below it too |
| GET    | `/api/schema`              | The derived schema of a subject with its drift |
