...
async function main() {
  ...
  const interval = setInterval(async () => {
    ...
    nc.publish(
      ...
      jc.encode({
        x: +state.robot01.x.toFixed(1),
        y: +state.robot01.y.toFixed(1),
        z: +state.robot01.z.toFixed(1),
        a: drift(45, 2),
        b: drift(0, 1),
        c: drift(90, 2),
        unit: 'mm/deg',
    ...
    nc.publish(
      ...
      jc.encode({
        x: +state.robot02.x.toFixed(1),
        y: +state.robot02.y.toFixed(1),
        z: +state.robot02.z.toFixed(1),
        a: drift(30, 2),
        b: drift(0, 1),
        c: drift(180, 2),
        unit: 'mm/deg',
    ...
    nc.publish(
      ...
      jc.encode({
        rpm: Math.round(state.cnc01.rpm),
        load: +state.cnc01.load.toFixed(1),
        temperature: +drift(state.cnc01.temp, 0.5).toFixed(1),
        vibration: +state.cnc01.vibration.toFixed(2),
        timestamp: now,
      }),
    );
    ...
    if (tick % 2 === 0) {
      ...
      nc.publish(
        'uns.acme.factory-berlin.assembly.line-1.conveyor-01.speed',
        jc.encode({
          value: +drift(1.2, 0.05).toFixed(2),
          unit: 'm/s',
          setpoint: 1.2,
          timestamp: now,
        }),
      ...
      nc.publish(
        'uns.acme.factory-berlin.assembly.line-1.conveyor-01.status',
        jc.encode({
          state: 'running',
          items_count: state.conveyor01.items,
          jams_today: state.conveyor01.jams,
          timestamp: now,
        }),
      );
    }
    ...
    nc.publish(
      'uns.acme.factory-berlin.logistics.warehouse.agv-01.position',
      jc.encode({
        x: +state.agv01.x.toFixed(1),
        y: +state.agv01.y.toFixed(1),
        floor: 1,
        zone: 'A',
        heading: +state.agv01.heading.toFixed(1),
        timestamp: now,
      }),
    );
    nc.publish(
      ...
      jc.encode({
        ...
        battery: +state.agv01.battery.toFixed(1),
        speed: +drift(0.8, 0.2).toFixed(1),
        payload_kg: randInt(0, 80),
        destination: pick(['line-1.input', 'line-2.input', 'warehouse.staging']),
        timestamp: now,
      }),
    );
    ...
    if (tick % 5 === 0) {
      ...
      nc.publish(
        'uns.acme.factory-berlin.assembly.line-1.sensor-temp-01.value',
        jc.encode({
          value: +state.tempSensor01.value.toFixed(1),
          unit: 'celsius',
          quality: 'good',
          timestamp: now,
        }),
      ...
      nc.publish(
        'uns.acme.factory-berlin.machining.line-2.sensor-temp-02.value',
        jc.encode({
          value: +state.tempSensor02.value.toFixed(1),
          unit: 'celsius',
          quality: 'good',
          timestamp: now,
        }),
      ...
      nc.publish(
        'uns.acme.factory-berlin.assembly.line-1.robot-01.temperature',
        jc.encode({
          motor1: +state.robot01.motor1.toFixed(1),
          motor2: +state.robot01.motor2.toFixed(1),
          motor3: +state.robot01.motor3.toFixed(1),
          controller: +drift(35, 1).toFixed(1),
          unit: 'celsius',
          timestamp: now,
        }),
      );
      ...
      nc.publish(
        ...
        jc.encode({
          state: 'running',
          mode: 'auto',
          uptime_h: 1247 + tick / 3600,
          cycle_count: state.robot01.cycles,
          last_error: null,
          timestamp: now,
        }),
      );
      ...
      nc.publish(
        'uns.acme.factory-berlin.assembly.line-1.robot-02.status',
        jc.encode({
          state: 'running',
          mode: 'auto',
          uptime_h: 983 + tick / 3600,
          cycle_count: state.robot02.cycles,
          last_error: null,
          timestamp: now,
        }),
      );
      ...
      nc.publish(
        'uns.acme.factory-berlin.machining.line-2.cnc-01.status',
        jc.encode({
          state: 'running',
          mode: 'auto',
          program: 'OP-4523',
          parts_completed: state.cnc01.parts,
          tool_wear: +state.cnc01.toolWear.toFixed(1),
          timestamp: now,
        }),
      );
      ...
      // Coolant
      nc.publish(
        'uns.acme.factory-berlin.machining.line-2.cnc-01.coolant',
        jc.encode({
          flow: +drift(14.8, 0.5).toFixed(1),
          temperature: +drift(21.3, 0.3).toFixed(1),
          level: +drift(78, 1).toFixed(0),
          unit: 'l/min | celsius | percent',
          timestamp: now,
        }),
      );
      ...
      nc.publish(
        ...
        jc.encode({
          active_kw: +state.energy.power.toFixed(1),
          reactive_kvar: +drift(87, 5).toFixed(1),
      ...
      nc.publish(
        'uns.acme.factory-berlin.environment.hvac.temperature',
        jc.encode({
          indoor: +state.hvac.indoor.toFixed(1),
          outdoor: +drift(state.hvac.outdoor, 0.2).toFixed(1),
          setpoint: 22.0,
          unit: 'celsius',
          timestamp: now,
        }),
      ...
      nc.publish(
        'uns.acme.factory-berlin.environment.hvac.humidity',
        jc.encode({
          indoor: +state.hvac.humidity.toFixed(1),
          outdoor: +drift(78, 2).toFixed(1),
          setpoint: 45.0,
          unit: 'percent',
          timestamp: now,
        }),
      ...
      // PLC status
      nc.publish(
        'uns.acme.factory-berlin.assembly.line-1.plc-01.status',
        jc.encode({
          state: 'run',
          scan_time_ms: +drift(2.3, 0.2).toFixed(1),
          memory_usage: randInt(40, 55),
          io_errors: 0,
          timestamp: now,
        }),
      );
      nc.publish(
        'uns.acme.factory-berlin.machining.line-2.plc-02.status',
        jc.encode({
          state: 'run',
          scan_time_ms: +drift(1.8, 0.15).toFixed(1),
          memory_usage: randInt(35, 45),
          io_errors: 0,
          timestamp: now,
        }),
      );
    ...
    if (tick % 30 === 0) {
      ...
      nc.publish(
        ...
        jc.encode({
          batch_id: 'B-2025-0316-001',
          product: 'Assembly-A',
          started: '2025-03-16T06:00:00Z',
          produced: state.line1.produced,
          rejected: state.line1.rejected,
          quality: +((1 - state.line1.rejected / state.line1.produced) * 100).toFixed(1),
          timestamp: now,
        }),
      );
      ...
      nc.publish(
        ...
        jc.encode({
          batch_id: 'B-2025-0316-002',
          product: 'Part-X7',
          started: '2025-03-16T06:00:00Z',
          produced: state.line2.produced,
          rejected: state.line2.rejected,
          quality: +((1 - state.line2.rejected / state.line2.produced) * 100).toFixed(1),
          timestamp: now,
        }),
      );
      ...
      nc.publish(
        'metrics.oee.line-1',
        jc.encode({
          oee: +drift(87.3, 2).toFixed(1),
          availability: +drift(95.2, 1).toFixed(1),
          performance: +drift(91.8, 2).toFixed(1),
          quality: +drift(99.8, 0.3).toFixed(1),
          timestamp: now,
        }),
      );
      nc.publish(
        'metrics.oee.line-2',
        jc.encode({
          oee: +drift(92.1, 1.5).toFixed(1),
          availability: +drift(97.5, 0.8).toFixed(1),
          performance: +drift(94.5, 1.5).toFixed(1),
          quality: +drift(99.9, 0.2).toFixed(1),
          timestamp: now,
        }),
      );
      nc.publish(
        'metrics.energy.factory',
        jc.encode({
          consumption_kw: +state.energy.power.toFixed(1),
          cost_eur_h: +(state.energy.power * 0.12).toFixed(2),
          co2_kg_h: +(state.energy.power * 0.4).toFixed(1),
          timestamp: now,
        }),
      );
      nc.publish(
        'metrics.production.line-1',
        jc.encode({
          rate: state.line1.rate,
          target: 120,
          efficiency: +((state.line1.rate / 120) * 100).toFixed(1),
          timestamp: now,
        }),
      );
      nc.publish(
        'metrics.production.line-2',
        jc.encode({
          rate: state.line2.rate,
          target: 85,
          efficiency: +((state.line2.rate / 85) * 100).toFixed(1),
          timestamp: now,
        }),
      );

      nc.publish(
        'uns.acme.factory-berlin.energy.main-meter.consumption',
        jc.encode({
          today_kwh: +state.energy.todayKwh.toFixed(1),
          month_kwh: +(48923 + state.energy.todayKwh).toFixed(1),
          year_kwh: +(289345 + state.energy.todayKwh).toFixed(1),
          timestamp: now,
        }),
      );
    }
    ...
    if (Math.random() < 0.005) {
      ...
      nc.publish(
        ...
        jc.encode({
          alarm_id: 'ALM-004',
          severity: vib > 4.5 ? 'critical' : 'warning',
    ...
    if (Math.random() < 0.01) {
      nc.publish(
        'events.production.line-1.quality-check',
        jc.encode({
    ...
    if (tick % 60 === 0) {
      nc.publish(
        'audit.system.heartbeat',
        jc.encode({
          source: 'uns-simulator',
          uptime_s: tick,
          messages_published: tick * 15,
          timestamp: now,
        }),
      );
    }
