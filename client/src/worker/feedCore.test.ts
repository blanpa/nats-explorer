...
describe('FeedCore', () => {
  it('lays the tree out once per batch of updates', () => {
    const h = harness();
    h.core.view = { all: false, paths: ['orders'], filterCollapsed: [], filter: '', expr: '', hideSystem: true, preview: false };
  ...
  it('shows system roots when the filter asks for them', () => {
    ...
    h.core.view = { all: false, paths: [], filterCollapsed: [], filter: '$JS', expr: '', hideSystem: true, preview: false };
    h.core.layoutTree();
  ...
  it('shows every match of a filter, but lets a branch be closed again', () => {
    const h = harness();
    h.core.handleEvent({
      type: 'subject-tree',
      connId: 'a',
      full: true,
      data: [entry('uns'), entry('uns.robot'), entry('uns.robot.position'), entry('uns.robot.status')],
    } as WsServerEvent);

    // Filtering opens the matches: typing a filter and seeing nothing but
    // roots would be useless.
    h.core.view = { all: false, paths: [], filterCollapsed: [], filter: 'robot', expr: '', hideSystem: true, preview: false };
    h.core.layoutTree();
    expect(h.lastTree()?.rows.map(r => r.subject)).toEqual(['uns', 'uns.robot', 'uns.robot.position', 'uns.robot.status']);

    // Open by default is not the same as unclosable.
    h.core.view = { ...h.core.view, filterCollapsed: ['uns.robot'] };
    h.core.layoutTree();
    expect(h.lastTree()?.rows.map(r => r.subject)).toEqual(['uns', 'uns.robot']);

    // And a collapsed root takes its whole subtree with it.
    h.core.view = { ...h.core.view, filterCollapsed: ['uns'] };
    h.core.layoutTree();
    expect(h.lastTree()?.rows.map(r => r.subject)).toEqual(['uns']);
  });
});
