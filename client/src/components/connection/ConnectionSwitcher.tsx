...
import { Check, ChevronDown, Loader2, Package, Plug, Settings2, Unplug } from 'lucide-react';
...
export default function ConnectionSwitcher() {
  ...
  const openBundle = async (file: File) => {
    setOpening(true);
    try {
      const res = await bundleApi.open(file);
      setActiveConnId(res.id);
      toast.success('Bundle opened', `${res.name} · ${res.messages} messages`);
    } catch (err) {
      toast.error('Could not open the bundle', errorMessage(err));
    } finally {
      setOpening(false);
    }
  };
        ...
        >
          {active ? (
            <>
              ...
              {active.bundle ? (
                <span className="badge badge-neutral shrink-0">bundle</span>
              ) : (
      ...
      <DropdownMenu.Portal>
        ...
        >
                ...
                >
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    <span className="block truncate text-xs text-muted font-mono">
                      {c.bundle ? 'imported bundle' : c.server?.replace(/^nats:\/\//, '') || c.servers?.[0]}
          ...
          {canWrite && (
            <DropdownMenu.Item
              onSelect={e => {
                // The dialog would close before the file picker opens.
                e.preventDefault();
            ...
            >
              {opening ? <Loader2 size={13} className="animate-spin" /> : <Package size={13} />} Open support bundle…
            </DropdownMenu.Item>
          )}
          <DropdownMenu.Item
            onSelect={() => openDialog()}
